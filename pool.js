...
const BALL_RADIUS_SCALE = 0.0215;
...
function makeBall(x, y, color, type = "target") {
  return {
    ...
    vx: 0,
    vy: 0,
    r: Math.max(BALL_RADIUS_MIN, Math.round(Math.min(table.w, table.h) * BALL_RADIUS_SCALE)),
    color,
    type,
    sunk: false,
  };
...
function placeBallWithoutOverlap(type, color) {
  const radius = Math.max(BALL_RADIUS_MIN, Math.round(Math.min(table.w, table.h) * BALL_RADIUS_SCALE));
