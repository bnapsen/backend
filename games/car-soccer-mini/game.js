(() => {
  ...
  function updateCar(car, dt) {
    ...
    const accel = 720;
    const turnPower = 2.35;
    const maxSpeed = 560;
    const maxTurnRate = 0.065;
    ...
    car.angVel += input.steer * turnPower * (0.5 + Math.min(speed / maxSpeed, 1) * 0.55) * dt;
    car.angVel *= input.handbrake ? 0.88 : 0.74;
    car.angVel = clamp(car.angVel, -maxTurnRate, maxTurnRate);
