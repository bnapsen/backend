(() => {
  ...
  function drawPickup(pickup) {
    ...
    if (pickup.type === 'sim-coin') {
      ...
      const collectRadius = pickup.collectRadius || pickup.r + 54;
      const pulse = Math.max(0, Math.sin(time)) * 4;
      ctx.translate(pickup.x, pickup.y);
      ctx.strokeStyle = 'rgba(255, 209, 102, 0.22)';
      ctx.lineWidth = 2;
      ctx.setLineDash([9, 13]);
      ctx.beginPath();
      ctx.arc(0, 0, collectRadius, 0, Math.PI * 2);
      ctx.stroke();
      ctx.setLineDash([]);
      ctx.fillStyle = 'rgba(255, 209, 102, 0.12)';
      ctx.beginPath();
      ctx.arc(0, 0, Math.min(collectRadius, pickup.r + 54) + pulse, 0, Math.PI * 2);
      ctx.fill();
      ctx.translate(0, bob);
      ...
      ctx.fillStyle = 'rgba(255, 209, 102, 0.18)';
      ctx.beginPath();
      ctx.arc(0, 0, pickup.r + 12 + pulse, 0, Math.PI * 2);
      ctx.fill();
      ctx.scale(flip, 1);
